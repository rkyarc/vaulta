...
export default function CreateTransactionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Sesuaikan tipe data sebelum dikirim ke API
      const payload = {
        type: formData.type,
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input type="number" name="amount" value={formData.amount} onChange={handleChange} required min="1" placeholder="Contoh: 50000" className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select name="categoryId" value={formData.categoryId} onChange={handleChange} required className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all bg-white">
                ...
                {/* Opsi statis untuk berjaga-jaga jika API belum siap, sesuaikan ID-nya dengan milikmu */}
                <option value="7">Makanan (ID: 7)</option>
          ...
          <div>
            ...
            <input type="text" name="description" value={formData.description} onChange={handleChange} required placeholder="Contoh: Makan siang di kantin" className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" />
          </div>
